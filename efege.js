...
const Xinz = 'XinzBot'
const Pkode = 'pais'
const lolkey = 'e2c14ab01e67ef9820eced22'  //est� api puedecar, si tiene uno reemplacelo
...

 *Pide las reglas del grupo*, O escribe /

 Escriba / para comenzar a usar el Bot.`

const moment = require('moment-timezone')
const jm = moment.tz('America/La_Paz').format('HH:mm:ss')
let d = new Date
let locale = 'es'
...
try {
pushnem = mek.key.fromMe ? Fg.user.name : conts.notify || conts.vname || conts.name || '-'
...
exec(`magick './image/wel.jpg' -gravity west -fill '#00FFFF' -font './image/font-gue.ttf' -size 1280x710 -pointsize 75 -interline-spacing 7.5 -annotate +460-45 '${pushnem}' -pointsize 35 -annotate +460+83 '${jm} ${calender}' -pointsize 50 -annotate +460+200 'Grupo > ${mdata.subject}' '${ppimg}' -resize %[fx:t?u.w*0.2:u.w]x%[fx:?u.h*0.2:u.h] -gravity center -geometry -430+70 -composite 'hamsil.jpg'`)
.on('error', () => reply('error'))
.on('exit', () => {
Fg.sendMessage(mdata.id, fs.readFileSync('hamsil.jpg'), MessageType.image, {caption: teks, contextInfo: { "mentionedJid": [num] }})
})
...
exec(`magick './image/salida.jpg' -gravity west -fill '#00FFFF' -font './image/font-gue.ttf' -size 1280x710 -pointsize 75 -interline-spacing 7.5 -annotate +460-45 '${encodeUrl(namaewa)}' -pointsize 35 -annotate +460+83 '${jm} ${calender}' -pointsize 50 -annotate +460+200 'Sali� de > ${mdata.subject}' '${ppimg}' -resize %[fx:t?u.w*0.2:u.w]x%[fx:?u.h*0.2:u.h] -gravity center -geometry -430+70 -composite 'hamsil.jpg'`)
.on('error', () => reply('error'))
.on('exit', () => {
Fg.sendMessage(mdata.id, fs.readFileSync('hamsil.jpg'), MessageType.image, {caption: teks, contextInfo: { "mentionedJid": [num] }})
})
...

 *Nombre* : @${num.split('@')[0]}
 *N�mero* : ${num.replace('@s.whatsapp.net', '')}
 *Info* : ${thu.status} 
...
if (isGroup) {
  ...
  Fg.sendMessage(from, `Maaf syarat member harus di atas ${memberlimit}, selamat tinggal 👋🏻`, text)
