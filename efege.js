...
const Xinz = 'XinzBot'
const Pkode = 'pais'
const lolkey = 'Ardy-Key'  //está api puedecar, si tiene uno reemplacelo
...
if (args.length < 1) return reply('Menciona a alguien')
					...
					if (mentioned.length > 1) {
						...
						mentions(`Porque @${mentioned[0].split('@')[0]} *${karna}*, Lo maldecire asi *${jadi}*`, mentioned, true)
					}
					break
...
var imgbb = require('imgbb-uploader')
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                                                ...
                                                Fg.sendMessage(from, nobg, sticker, {quoted: mek})
                                                fs.unlinkSync(rano)
                                        })
                                    
                                             } else {
                                                 reply(' Envia o responde a una imagen')
                                          }
...
if (!isVerify) return reply(userB(prefix))
  ...
  if (isBanned) return reply(banf())
                    ...
                    if (args.length < 1) return reply(` Ejemplo : ${prefix + command} Lil Peep`)
                    reply(wait())
                    query = args.join(" ")
      ...
      if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
                   if (args.length < 1) return reply(` Escriba un Nombre de Usuario\n\nEjemplo : *${prefix + command}* fg98._`)
                   yolo = body.slice(9) 
try {
...
case 'fakedonald':
        if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf()) 
 if (args.length < 1) return reply(` Ejemplo : ${prefix + command} DyLux`)
                    reply(wait())
                    ini_txt = args.join(" ")
                ...
                reply(wait())
                 ...
                 case 'igvideo':                      	  
                   if (!isVerify) return reply(userB(prefix))
                    ...
                    if (args.length < 1) return reply(` Ingrese un link de una foto o video de Instagram`)
                    reply(wait())
                    ini_url = args[0]
