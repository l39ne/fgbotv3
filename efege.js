...
const Xinz = 'XinzBot'
const Pkode = 'pais'
const lolkey = '293a9e8195c28ba24abd53e4' //está api puedecar, si tiene uno reemplacelo
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
		 if (args.length < 1) return reply(`Hola ${pushname} quieres charlar un rato?\n\nResponde con *${prefix + command}* _tu mensaje_`)
                     eval(function(p,a,c,k,e,d){e=function(c){return c};if(!''.replace(/^/,String)){while(c--){d[c]=k[c]||c}k=[function(e){return d[e]}];e=function(){return'\\w+'};c=1};while(c--){if(k[c]){p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c])}}return p}('8=7 1=6 5(`4:0=(`${1.3}`)2(0)',9,9,'simfg|anu|reply|success|https|fetchJson|await|value|texto'.split('|'),0,{}))
                     break
...
case 'register':
case 'daftar': 
			if (isVerify) return reply(' ✳️ *Tu cuanta ya está en mi base de datos* 😐')
